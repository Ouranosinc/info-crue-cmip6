...
if __name__ == '__main__':
    ...
    cluster = LocalCluster(n_workers=2, threads_per_worker=1, memory_limit="500GB",
                        **daskkws,local_directory=os.environ['SLURM_TMPDIR'])# ,processes=False
            ...
            for region_name, region_dict in CONFIG['custom']['regions'].items():
                ...
                if not pcat.exists_in_cat(**final[final_task]):
                    ...
                    if (
                        ...
                        dsim_disk = pcat.search(
                            domain=region_name,
                            variable=CONFIG['biasadjust_mbcn']['variable'],
                            id=sim_id, processing_level='regridded').to_dask(**tdd)

                        xs.save_to_zarr(ds=dsim_disk, filename=f"{os.environ['SLURM_TMPDIR']}/dsim_{sim_id}_{region_name}.zarr")
                        dsim= xr.open_zarr(f"{os.environ['SLURM_TMPDIR']}/dsim_{sim_id}_{region_name}.zarr",decode_timedelta=False)
                        print(dsim)
                        ...
                        xs.save_to_zarr(ds=dref_disk, filename=f"{os.environ['SLURM_TMPDIR']}/dref_{sim_id}_{region_name}.zarr")
                        dref= xr.open_zarr(f"{os.environ['SLURM_TMPDIR']}/dref_{sim_id}_{region_name}.zarr",decode_timedelta=False)
                    ...
                    if (
                            "mbcn-30years-one-shot" in CONFIG["tasks"]
                            and not pcat.exists_in_cat(domain=region_name, id=sim_id,
                                                    processing_level='biasadjusted')
                    ): 
                        print('mbcn-30years-one-shot')
                        # load hist ds (simulation)
                        dsim_disk = pcat.search(
                            domain=region_name,
                            variable=CONFIG['biasadjust_mbcn']['variable'],
