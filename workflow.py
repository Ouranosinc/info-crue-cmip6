    ...
    if "ensembles" in CONFIG["tasks"]:
            ...
            if not pcat.exists_in_cat(processing_level=ens_name):
                with (
                    ...
                    ds_ens = xs.ensemble_stats(
                        datasets=datasets, weights=weights, to_level=ens_name
                    )

                    # save and update
                    save_and_update(
                        ds_ens,
                        CONFIG["paths"]["ensembles"],
                        pcat,
        ...
        for diff_name, diff_inputs in CONFIG["ensembles"]["diffs"].items():
            if not pcat.exists_in_cat(processing_level=diff_name):
                with (
                    Client(
                        n_workers=4, threads_per_worker=5, memory_limit="6GB", **daskkws
                    ),
            ...
            if not pcat.exists_in_cat(processing_level=p_name):
                with (
                    ...
                    pvals = xr.apply_ufunc(
                        lambda f, r: scipy.stats.ttest_ind(
                            f, r, axis=-1, nan_policy="omit", equal_var=False
                        ).pvalue,
                        ens1,
                        ens2,
